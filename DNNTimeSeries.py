...
class DNN:
	...
	def __init__(self, x_training_set, y_training_set, x_dev_set, y_dev_set, x_test_set, y_test_set, time_steps, action, output_multi):
		...
		self.history = None
		self.batch_size = 30	#2
		self.epochs = 150	#100	#50
		self.dropout = 0.3	#0.2	#0.5
		self.neurons = 130	#15 #200		#60	#8
    ...
    # funzione che crea il modello di learning con un layer aggiuntivo
    # parametri:
    # 	- neurons: neuroni nuovo hidden layer
    # 	- dropout: parametro di regolarizzazione
	def create_deep_model(self, neurons=-1, dropout=-1):
		...
		if dropout > 0.0:  # se voglio il dropout
			self.model.add(Dropout(dropout, input_shape=(self.time_steps,)))
			self.model.add(Dense(300, activation='relu'))
		else:  # se non lo voglio
			self.model.add(Dense(neurons, input_shape=(self.time_steps,), activation='relu'))
		...
		self.model.compile(loss='mean_squared_error', optimizer='adam')		#, metrics=[self.custom_metric_rsquared])
		return self.model
	...
	def tune_neurons(self):
		...
		model = KerasRegressor(build_fn = self.create_deep_model, epochs = self.epochs, batch_size = self.batch_size, verbose=2)
	...
	def tune_batch_size_epochs(self):
		...
		epochs = [50, 100]#, 150]	#[10, 50, 100, 200]
	...
	def train_model(self):
		...
		# creo il modello della DNN
		self.model = self.create_model()           #self.create_deep_model()
		...
		fig.savefig("Training_DNN_loss.png")
		plt.close(fig)
		return self.model
